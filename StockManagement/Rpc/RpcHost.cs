...
namespace Rpc
{
	public class RpcHost : IDisposable
	{
		...
		private RpcHost(IPEndPoint endPoint)
		{
		...
		public RpcHost(Type type, IPEndPoint endPoint)
			: this(endPoint)
		{
			this.type = type;
		}

		public RpcHost(Type type, string endpoint)
			: this(type, new IPEndPoint(IPAddress.Parse(endpoint.Split(':')[0]), int.Parse(endpoint.Split(':')[1])))
		{
		}


		public RpcHost(object service, IPEndPoint endpoint)
			: this(endpoint)
		{
			if (service != null)
			{
		...
		public RpcHost(object service, string endpoint)
